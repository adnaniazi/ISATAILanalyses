...
#loadd(rep4_roos_30bp)
#rep1_ff_tf = rep4_roos_30bp # because all the code below is using this stupid name

loadd(rep4_roos_100bp)
rep1_ff_tf = rep4_roos_100bp # because all the code below is using this stupid name

```

# About the data
Here is a description of columns of `rep4_roos_100bp`:
```{r data-description-1, echo=FALSE}
col_names_df <- data.frame(Columns = c("start", "end", "fastq_segment", "read_type", "cluster", "tail_start", "tail_end", "read_id", "cluster_n", "file_path", "tail_length", "samples_per_nt"), Description = c("Where does the FASTQ segment start in the FASTQ sequence of the entire read", "Where does the FASTQ segment end in the FASTQ sequence of the entire read", "Sequence of the FASTQ segment itself", "Read type (polyA, polyT, consensus_polyA, consensus_polyT)", "Cluster # the read belongs to", "Sample index of the start of the tail", "Sample index of the end of the tail", "Read ID", "Number of repeats present in a cluster which were used to compute the mean tail estimate", "Full filepath (only relevant within Valen Lab)", "Tail length in nt", "Translocation rate calculated over the entire long read"))                                                                                                                                
```
...
read_count <- length(unique(rep1_ff_tf$read_id))
cat(paste0("Read count: ", read_count,  " reads.", '\n'))
```

## Filter invalid reads
```{r, warning=FALSE}
filtered_data <- rep1_ff_tf %>% 
  filter(read_type == 'consensus_polyA' | read_type == 'consensus_polyT')
cat(paste0("Reads left after filtering invalid reads: ", 
           length(unique(filtered_data$read_id)), '\n'))

total_clusters_before_filterting <- nrow(filtered_data)
cat(paste0("Clusters found in these remaining reads: ", 
           total_clusters_before_filterting, '\n'))

```

## Invalid clusters
Some clusters contain only one segment, and that segment does not have a polyA or a polyT low-variance segment. 
```{r, warning=FALSE}
filtered_data <- filtered_data %>% filter(cluster_n !=0 )
total_clusters_after_filterting <- nrow(filtered_data)
cat(paste0("Clusters left after removing erroneous clusters that do not contain a tail region: ", 
           total_clusters_after_filterting, '\n'))

```


```{r, warning=FALSE}
cat(paste0("Number of reads from which these remaining clusters originated: ", 
           length(unique(filtered_data$read_id)), '\n'))

```

## Number of reads with both PolyA and PolyT tails in them
```{r, warning=FALSE}
# summarize the data and display a table
summary <- filtered_data %>% group_by(read_id, read_type) %>% 
  summarize(n = n()) %>% 
  ungroup()
before <- nrow(summary)

summary2 <- spread(summary, key = read_id, value = n)
after <- ncol(summary2)

cat(paste0("Number of reads with both polyA and polyT segments: ", 
           before - after, '\n'))
```

## Clusters per read
How many reads have 1, 2, or 3 clusters. It is a bit confusing. I will explain when I meet today.


```{r, warning=FALSE}
# summarize the data and display a table
summary_data <- filtered_data %>% 
  group_by(read_type, cluster) %>% 
  summarize(Reads = n())
kable(summary_data)
```


## Tail length distribution (Consensus)

```{r, warning=FALSE}
p <- ggplot(filtered_data, aes(x = read_type, y = tail_length,
                                    color = read_type, fill = read_type)) +
  geom_two_sided_flat_violin(position = position_nudge(x = 0, y = 0), alpha = .5) 
```

```{r, warning=FALSE, echo=FALSE}
lengend_name <- 'Tail type'
legend_labels <- c('Consensus Poly(A)',
                   'Consensus Poly(T)')
p <- p + theme(panel.background = element_blank(),
        ...
        legend.position = "top") +
  geom_hline(yintercept = c(30, 100)) +
  ...
  labs(caption = 'Consensus implies that the tail length was computed as a mean 
       of the tail length estimate of each individual segment within the cluster of segments.
       The black lines represent 30nt and 100nt.')
p
```

## Tail length distribution (Non-Consensus i.e. for individual segments)

```{r, warning=FALSE}
non_consensus_data <- rep1_ff_tf %>% 
  filter(read_type == 'polyA' | read_type == 'polyT') %>% 
  filter(!is.na(tail_length))
```

```{r, warning=FALSE}
p <- ggplot(non_consensus_data, aes(x = read_type, y = tail_length,
                                    color = read_type, fill = read_type)) +
  geom_two_sided_flat_violin(position = position_nudge(x = 0, y = 0), alpha = .5) 
```
...
```{r, warning=FALSE, echo=FALSE}
lengend_name <- 'Tail type'
legend_labels <- c('Poly(A) Segments',
                   'Poly(T) Segments')
p <- p + theme(panel.background = element_blank(),
        panel.grid.minor = ggplot2::element_line(colour = "grey", size = 0.5),
        panel.grid.major = ggplot2::element_line(colour = "grey", size = 0.5),
...
## What is the tail length for polyA and polyT within the same read
```{r, warning=FALSE}
# summarize the data and display a table
summary <- filtered_data %>% group_by(read_id, read_type) %>% 
  summarize(n = n()) %>% 
...
summary2 <- spread(summary, key = read_id, value = n)
summary2 <- data.frame(t(summary2))

summary2 <- summary2[complete.cases(summary2), ]

summary2 <-  summary2[-c(1), ] 
summary2 <- add_rownames(summary2, var = "read_id")

colnames(summary2) <- c('read_id', 'consensus_polyA', 'consensus_polyT')

polyat_data <- inner_join(non_consensus_data, summary2, by = 'read_id')

```

```{r, warning=FALSE}
p <- ggplot(polyat_data, aes(x = read_type, y = tail_length,
                                    color = read_type, fill = read_type)) +
  geom_two_sided_flat_violin(position = position_nudge(x = 0, y = 0), alpha = .5) 
```
